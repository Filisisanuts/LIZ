...
function renderExpCalendar(){
    ...
    for(var d=1;d<=daysInMonth;d++){
        ...
        h+='<div style="background:'+(has?'var(--card)':'var(--card-h)')+';border:1px solid '+(isToday?'var(--ac)':'var(--bd)')+';border-radius:6px;padding:6px;cursor:'+(has?'pointer':'default')+';min-height:65px"';
...
function expCalNav(dir) {
    var parts = _expCalYM.split('-').map(Number);
    parts[1] += dir;
    if (parts[1] < 1) { parts[0]--; parts[1] = 12; }
    if (parts[1] > 12) { parts[0]++; parts[1] = 1; }
    _expCalYM = parts[0] + '-' + String(parts[1]).padStart(2, '0');
    var picker = $id('expCalPicker');
    if (picker) picker.value = _expCalYM;
    renderExp();
}
...
function printExpPDF() {
    ...
    html += '<div class="header"><h1>岸香咖啡 · 费用报告</h1>';
    ...
    html += '<div class="footer">岸香咖啡弘皓大厦店 · ' + td() + ' 生成</div></body></html>';
