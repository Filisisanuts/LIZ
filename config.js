// config.js - 数据库定义、配置、工具函数

// ==================== 1. 数据库定义 + 配置 ====================
// 存储键名
var STORE = 'ax_cafe_v8';

// 库存类型配置（茗茶/香烟/酒类），提前定义避免引用报错
var INV = {
...
var NAV = [
    ...
    { sep: true },
    { id: 'data',label: '设置',icon: '⚙️'}
];

// 数据库初始化模板
function initDB() {
    return { dailyReports:[],purchases:[],expenses:[],teaItems:[],cigItems:[],alcItems:[],areaCats:{},aiHistory:[],whItems:[],damageRecords:[] };
}
...
// 通用更新方法：执行回调 → 保存 → 触发云同步
function upd(fn){fn(DB);DB._ts=Date.now();saveDB(DB);sbScheduleSave()}
...
function closeModal(){var m=$id('modal'),o=$id('overlay');if(m)m.classList.remove('show');if(o){o.classList.remove('show');o.onclick=null}}
